...
async function test() {
  ...
  try {
    ...
    const teamName = (teamRes.teamName as string) || 'test-team'

    // 2. Listar times
    console.log('\n2️⃣  GET /api/swarm/teams - Listando times...')
    const listRes = await request('GET', '/api/swarm/teams')
    console.log('✅ Times encontrados:', (listRes.count as number) || 0)
    ...
    console.log('✅ Mensagens na mailbox:', (mailboxRes.count as number) || 0)
